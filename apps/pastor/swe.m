...
%axis
eta_axis = [-2*dx Lx+2*dx -.05*max(eta) 1.5*max(eta)];
...
%initialize NN friction slope
Hu = calculate_edge_elevations(u_old,H_old);
gamma = zeros(N+1,1);
S_MD = zeros(N+1,1);
w = find(Hu > wd_tol);
S_MD(w) = bingham_friction_slope(Hu(w),u_old(w+1),g,rho,tau_y,M,wd_tol);
...
while t <= t_f
    ...
    %treat non-Newtonian stress/friction explicitly
%     S_MD = zeros(N+1,1);
%     shear_rate = zeros(N+1,1);
%     [shear_rate(w)] = approx_shear_rate(u_old(w+1),Hu(w),flow_index);
%     S_MD(w) = bingham_friction_slope(Hu(w),u_old(w+1),g,rho,tau_y,M,wd_tol);
%     Gu = Gu - dt*g*S_MD;
...
[H_act] = analytical_soln(xc,xs,tau_y,rho,g); 
...
legend('numerical','actual','bathymetry','actual','FontSize',fs)

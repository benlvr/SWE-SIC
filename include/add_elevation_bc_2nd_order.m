function [T,b] = add_elevation_bc_2nd_order(Hu,eta_old,PHI,T,b,g,dt,theta,g_eta,bdry)
...
%change entry in T
T(c_idx,c_idx) = T(c_idx,c_idx) + 2*g*dt^2*theta^2*Hu(e_idx)*PHI(e_idx);

%add entry to the RHS of the nonlinear system
b(c_idx) = b(c_idx) + 2*g*dt^2*theta^2*Hu(e_idx)*PHI(e_idx)*g_eta;
